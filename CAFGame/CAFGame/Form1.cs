...
namespace CAFGame
{
    public sealed partial class Form1 : Form
    {
        ...
        private static void StartMenu()
        {
            ...
            Environment.MenuButtons.Add(new PlayButton(new Vector2(
                ...
                Environment.ScreenResolution.Height / 2f + 100 * Environment.ScreenResolutionMultiplier.Y)));
            Environment.MenuButtons.Add(new QuitButton(new Vector2(
                124 * Environment.ScreenResolutionMultiplier.X +
                (Environment.ScreenResolution.Width - (124 + 436) * Environment.ScreenResolutionMultiplier.X) / 2f,
                Environment.ScreenResolution.Height / 2f + 175 * Environment.ScreenResolutionMultiplier.Y)));
            Environment.MenuButtons.Add(new MuteSoundButton(new Vector2(
                Environment.PosRangeBottomRight.X - 50 * Environment.ScreenResolutionMultiplier.X,
                Environment.PosRangeTopLeft.Y + 40 * Environment.ScreenResolutionMultiplier.Y)));
        }
        ...
        {
            Environment.desktopLocation = DesktopLocation;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (Environment.GameState == GameState.MainMenu)
            {
                ...
                var p = new Vector2(
                    124 * Environment.ScreenResolutionMultiplier.X +
                    (Environment.ScreenResolution.Width - (124 + 436) * Environment.ScreenResolutionMultiplier.X) / 2f,
                    Environment.ScreenResolution.Height / 2f - 200 * Environment.ScreenResolutionMultiplier.Y);
            ...
            if (Environment.GameState == GameState.Game)
            {
                foreach (var t in Environment.Enemies)
                {
                    ...
                    {
                        Environment.RevolverDrum.Pos =
                            new Vector2(t.Pos.X + 35 * Environment.ScreenResolutionMultiplier.X,
                                t.Pos.Y + 30 * Environment.ScreenResolutionMultiplier.Y);
        ...
        {
            var pos = new Point((int) (100 * Environment.ScreenResolutionMultiplier.X),
                (int) (65 * Environment.ScreenResolutionMultiplier.Y));
        ...
        private void DrawPlatform(GameObject t, Graphics g)
        {
            ...
            var yPos = t.Pos.Y + 55 * Environment.ScreenResolutionMultiplier.Y -
                       Environment.Platform.Sprite.Size.Height / 2f;
            g.DrawImage(Environment.Platform.Sprite, xPos, yPos);
        }

        private void DrawSwitchGenreTimer(Graphics g)
        {
            var f = new Font("Impact",
                100 * (Environment.ScreenResolutionMultiplier.X + Environment.ScreenResolutionMultiplier.Y) / 2f);
            var b = new SolidBrush(Color.FromArgb(170, Color.White));
            var str = (20 - GenreChanger.GenreChangeTimer / 1000f).ToString("00");
            g.DrawString(str, f, b, 124 * Environment.ScreenResolutionMultiplier.X,
                121 * Environment.ScreenResolutionMultiplier.Y, new StringFormat());
        }

        private void DrawBestScore(Graphics g)
        {
            var f = new Font("Impact",
                25 * (Environment.ScreenResolutionMultiplier.X + Environment.ScreenResolutionMultiplier.Y) / 2f);
        ...
        {
            var f = new Font("Arial",
                65 * (Environment.ScreenResolutionMultiplier.X + Environment.ScreenResolutionMultiplier.Y) / 2f);
            var b = new SolidBrush(Color.Black);
            var a = Environment.ScreenResolution;
            g.DrawString(Score.ToString("0000"), f, b,
                Environment.ScreenResolution.Width - 298 * Environment.ScreenResolutionMultiplier.X,
                Environment.ScreenResolution.Height - 170 * Environment.ScreenResolutionMultiplier.Y,
                new StringFormat());
        }
        ...
        {
            var xPos = obj.Pos.X -
                       30 * Environment.ScreenResolutionMultiplier.X * ((i + 1) / 2) * (i % 2 == 0 ? 1 : -1) -
                       Environment.Heart.Sprite.Size.Width / 2f;
            var yPos = obj.Pos.Y - 50 * Environment.ScreenResolutionMultiplier.X -
                       Environment.Heart.Sprite.Size.Height / 2f;
